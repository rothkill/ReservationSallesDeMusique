...
//			st.executeUpdate();
////			return new Salle(idCategory,nom,tarifUneHeure,tarifDeuxHeures,nbPersonne);
...
//	}
	...
	public Salle rechercher(int idSalle){
		Salle salle = null;
		try {
			PreparedStatement st = con.prepareStatement("select idcategorie,nom,etat from salle where idsalle = ?");
			st.setInt(1, idSalle);
			ResultSet rs = st.executeQuery();
			if(rs.next() != false){
				salle = new Salle(idSalle,rs.getInt(1),rs.getString(2),rs.getString(3));
			}
